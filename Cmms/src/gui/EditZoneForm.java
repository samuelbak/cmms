...
public class EditZoneForm extends JFrame {
	...
	public EditZoneForm(final String[] path) {
		setTitle("Modifica zona");
		setResizable(false);
		...
		JLabel lblNomeDelNuovo = new JLabel("Nome della zona");
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(!textZoneName.getText().equals("")){
					try {
						editZone(path, textZoneName.getText());
	...
	private boolean editZone(String[] path, String text) throws SQLException{
		boolean ret = DatabaseOperation.editZones(path, text);
		MainForm.tree.clear();
		MainForm.tree.createTreeFromScratch();
		return ret;
	}
